    ...
    <div class="main-wrapper">
        ...
        <form action="" class="feedback-form" method="POST">
            ...
            <input type="submit" class="submit">
             <? } else { ?>
                <input type="text" name="comment" class="input" placeholder="Ваша почта ожидает подтверждения" autocomplete="off" disabled required>
        ...
        <? } else {
            ...
            foreach ($allComments as $comment) { ?>
                <div class="comment-section"><span class="name"><?= $comment['comment_name'] ?>:</span><span class="comment"><?= $comment['comment'] ?></span><span class="date"><?= substr($comment['comment_date'], 0, 10)?></span>
                    <?
                if ($is_admin == 1) {
                    ?> <form action="" method="POST"><button class="btn btn-danger" name="delete_comment" value="<?= $comment['id'] ?>" style="font-family:Arial,FontAwesome; padding:0 4px;">&#xf014;</button></form></div> <?
                } else {
                    ?> </div> <?
                }
            }
